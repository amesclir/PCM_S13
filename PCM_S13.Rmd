...
Los investigadores están haciendo uso de las filogenias y los modelos filogenéticos comparativos para entender mejor el proceso evolutivo de esamblage y mantinimiento de las comunidades.

La mayoría de estos análisis tratar de ver como se distribuyen las especies que forman comunidades en una región a lo largo de la filogenia. Normalemente tenemos un conjunto de especies de una region que se encuentran en 2 o más comunidades.

Si asumimos que las filogenias son surrogados de los caracteres funcionales de las especies (esto solo se puede asumir si tales rasgos funcionales tienen una señoal filogenética fuerte) se pueden obtener dos patrones diferentes en las comunidades, clustering y overdispersion (Webb et al. 2002). 
El patron de clustering se suele interpretar como el resultado del filtrado ambienteal (es decir, asumiendo que la filogenia es un surrogado de los caracters funcionales claves, solo las especies con ciertos estados de caracter clave pueden pasar ese filtrado ambiental, y por lo tanto solo algunas especies, filogeneticamente cercanas, estarán presentes esa comunidad después de un filtrado ambiental).
El patron de sobredispersión se suele interpretar como el resultado de la competencia fuerte entre especies cercanas (es decir, asumiendo que la filogenia es un surrogado de los caracters funcionales claves, las especies más cercanas filogeneticamente entre sí están en competencia y se excluyen unas a otras, por lo que solo las especies más filogeneticamente desemparentadas estarán presentes en este comunidad).
En realidad esto puede ser mucho más complejo. Si imaginamos que los caracteres funcionales clave no tiene señal filogenética, entonces esperamos que tras un proceso de filtrado ambiental encontremos un patrón al azar o sobredispersión, y en un proceso de exclusión por competencia esperarmos encontrar entonces un patrón de clustering en la filogenia.
...
Vamos a calcular nuestra primer indice de diversidad filogenética que no es otro que PD. En este caso lo que se hace es sumar todas las ramas de la filogenia de las especis presentes en cada comunidad.
De nuevo estamos asumiendo que diversidad de una comunidad la podemos medir con su diversidad funcional y que la filogenia es un surrogado de esta diversidad funcional.
...
Podemos observar tres columnas en la salida, una fila con los nombres de las comunidades, otra con la PD y una útlima con a riqueza de especies (SR). La riqueza de especies va de 1 a 169 especies y la PD en millones de años (la unidad de tiempo en las ramas de nuestra filogenia) varían en el orden de miles.
...
Ahora vamos a comprobar si en una comunidad concreta, en este caso Aleck_Rock hay clustering o overdispersion, es decir, si la PD es más pequeña o maś grande lo que cabrían esperar al azar. Para hacer esto creamos una distribución nula, mediante 999 randomizaciones. Es decir, miramos el número de presencias de Aleck_Rock y generamos al azar comunidades con el mismo número de presencia entre las 366 especies que hay en nuestro arbol, es decir, en nuestro pool regional.
...
En el paquete picante podemos encontrar la función pd.test que nos permite calcular la PD de todas la comunidades a la vez, generar sus distribuciones nulas y nos da valores de P para cada una de ellas y podemos saber automaticamente si estamos en un caso de clustering o sobredispersion.
...
Tenemos nuestro resultados. Tenemos los valores de P, que en este caso son de una sola cola. Es decir, cualquiera que sea menor de 0.05 o mayor de 0.95, muestra un valor significativo de clustering o overdispersion.
...
Todos estos calculos son de diversidad alpha pero también podemos calcular diversidad b con picante. Es decir, las diferencias entre las comunidades.
...
En la disciplina de filogenética de comunidades o filofloras es muy común no generar una filogenia a partir de secuencias de ADN sino generarla a partir de filogenias previamente inferidas y an algunos casos mediante el uso información de tipo taxonómica.
Para ello vamos a usar el paquete de R llamado randtip. Para instalar este paquete hace falta el paquete remotes.
Y vamos comprobar los nombre científicos con un paquete llamado U.Taxonstand.
...
#################
ACTIVIDADES
1. Repite todos los anáisis que hemos hecho para las comunidades de la Islas de San Juan pero para nuestro dataset
